...
def read_data(model_parameters, ARGS):
    ...
    data = pd.read_pickle(ARGS.path_data)['codes'].values
...
def lift(y_true, y_prob, graph):
    ...
    if graph:
        ...
        plt.style.use('ggplot')
        plt.clf()
        plt.plot(recall, lift_values, label='Lift-Recall Curve  (Area = %0.3f)' % average_lift)
        plt.xlabel('Recall: P(predicted+|true+)')
        plt.ylabel('Lift')
        plt.xlim([0.0, 1.0])
        plt.legend(loc='lower left')
...
def probability_calibration(y_true, y_prob,graph):
    if graph:
        ...
        fraction_of_positives, mean_predicted_value = calibration_curve(y_true, y_prob, n_bins=n_bins, normalize=True)
